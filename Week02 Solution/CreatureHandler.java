    ...
    */
    public void updateCreatures() {
        ...
        for(Creature c : _creatures) {
                ...
                // The new spawner behavior...
                if(c instanceof Spawner) {
                    Spawner s = (Spawner)c;
                    Creature baby = s.spawnNewCreature();

                    // The way we're looping through the list prevents us from adding creatures
                    // to it here. There are a few options to fix this, but one of the most 
                    // common is to just add them to a different list, then when we're done
                    // with the loop, we can loop through the new list and add each creature
                    // to the world.
                    if(baby != null)
                        babies.add(baby);
                }
            ...
            // Add all the babies to the world
